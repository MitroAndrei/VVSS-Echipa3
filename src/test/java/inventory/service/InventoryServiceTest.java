...
class InventoryServiceTest {
    ...
    void addPartEcpValid(){
        assertNull(service.lookupPart(name));
        service.addInhousePart(name, goodPrice, goodInStock, min, max, machineId);
        assertNotNull(service.lookupPart(name));
    }

    @Test
    void addPartEcpNonValid(){
        assertNull(service.lookupPart(name));
        assertThrows(ValidationException.class, () -> service.addInhousePart(name, -1.0, goodInStock, min, max, machineId));
        try {
            service.addInhousePart(name, -1.0, goodInStock, min, max, machineId);
            fail("Part added");
        } catch (ValidationException e) {
            assertEquals("The price must be greater than 0.0 ", e.getMessage());
            assertNull(service.lookupPart(name));
    ...
    @Test
    void addPartBvaValid(){
        assertNull(service.lookupPart(name));
        service.addInhousePart(name, 0, goodInStock, min, max, machineId);
        assertNotNull(service.lookupPart(name));
    }

    @Test
    void addPartBvaNonValid(){
        assertNull(service.lookupPart(name));
        assertThrows(ValidationException.class, () -> service.addInhousePart(name, -1.0, goodInStock, min, max, machineId));
        try {
            service.addInhousePart(name, -1.0, goodInStock, min, max, machineId);
